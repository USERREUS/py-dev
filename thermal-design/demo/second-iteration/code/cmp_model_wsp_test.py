...
def Technical_condition_tur(tвых_DT,tвых_real,hвых_DT,hвых_real,ОРР, ОС, Vib_h,Vib_v, Tem_oil):
    ...
    Tem_oil_max = 65.7
    Tem_oil_min = 0
    Z_ОРР = abs((ОРР_max - ОРР)/ (ОРР_max - ((ОРР_max - ОРР_min)*0.5)-((ОРР-(ОРР_max - ОРР_min)*0.5)/ОРР)*(ОРР_max - ОРР_min)*0.5))
    Z_ОС = abs((ОС_max - ОС) / (
                ОС_max - ((ОС_max - ОС_min) * 0.5) - ((ОС - (ОС_max - ОС_min) * 0.5) / ОС) * (ОС_max - ОС_min) * 0.5))
    Z_Vib_h = abs((Vib_h_max - Vib_h) / (
                Vib_h_max - ((Vib_h_max - Vib_h_min) * 0.5) - ((Vib_h - (Vib_h_max - Vib_h_min) * 0.5) / Vib_h) * (Vib_h_max - Vib_h_min) * 0.5))
    Z_Vib_v = abs((Vib_v_max - Vib_v)/ (Vib_v_max - ((Vib_v_max - Vib_v_min)*0.5)-((Vib_v-(Vib_v_max - Vib_v_min)*0.5)/Vib_v)*(Vib_v_max - Vib_v_min)*0.5))
    Z_Tem_oil = abs((Tem_oil_max - Tem_oil)/ (Tem_oil_max - ((Tem_oil_max - Tem_oil_min)*0.5)-((Tem_oil-(Tem_oil_max - Tem_oil_min)*0.5)/Tem_oil)*(Tem_oil_max - Tem_oil_min)*0.5))
    ...
    Txt= "Технического состояния оборудования - {0} Рекомендуемое техническое воздействие - {1}ИТС равен {2:1.3f}".format(verbal_assesment, recommend, ИТС)
    return Txt # Выходные данные

# Тестовые значения

print(Technical_condition_tur(180,185,2556.2,2656.2,2, 2, 2,2, 50))

print(Technical_condition_tur(180,185,2556.2,2456.2,4, 5, 5,2, 60))

print(Technical_condition_tur(180,192,2556.2,2256.2,4, 5, 5,6, 65))

print(Technical_condition_tur(180,185,2556.2,2356.2,4, 5, 5,6, 65))

print(Technical_condition_tur(180,185,2556.2,2456.2,4, 5, 5,6, 65))